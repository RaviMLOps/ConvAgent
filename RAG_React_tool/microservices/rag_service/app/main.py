...
def query_chroma_server(query: str, top_k: int = 3) -> List[Dict[str, Any]]:
    ...
    """
    try:
        print("Querying ChromaDB server...")
        print(CHROMA_QUERY_ENDPOINT)
...
async def rag_tool_search(input: QueryInput):
    ...
    try:
        ...
        try:
            # Ensure chunks is a list of strings or dictionaries with text field
            if chunks and isinstance(chunks, list):
                if isinstance(chunks[0], dict):
                    context_items = [chunk.get("text", "") if isinstance(chunk, dict) else str(chunk) for chunk in chunks]
                else:
                    context_items = [str(chunk) for chunk in chunks]
                context = "\n\n".join(context_items)
            else:
                context = str(chunks)
            ...
            if not rag_chain:
                raise ValueError("RAG chain not properly initialized")
            ...
            if not result:
                raise ValueError("No response generated by RAG chain")
            ...
            if isinstance(result, str):
                return {"response": result, "sources": chunks}
                ...
                return {"response": str(result), "sources": chunks}
                
        except Exception as e:
            print(f"Error in response generation: {str(e)}")
            return {"response": f"Error generating response: {str(e)}", "sources": []}
